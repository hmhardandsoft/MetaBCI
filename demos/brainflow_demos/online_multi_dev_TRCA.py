...
from metabci.brainda.algorithms.decomposition.cca import _trca_kernel,_trca_feature
import threading
import queue
    ...
    def predict(self, X: ndarray):
        try:
            if isinstance(X, list):
                ...
                if len(feat_) < 2:
                    labels = self.classes_[np.argmax(feat_, axis=-1)]

                else:
                    # 选择最大相关系数
                    new_feat = np.maximum.reduce(feat_)
                    print('new_feat', new_feat.shape)
                    labels = self.classes_[np.argmax(new_feat, axis=-1)]
            else:
                feat = self.transform(X)
                print('length feat2', feat.shape)
...
if __name__ == "__main__":
    ...
    brk_port = NeuraclePort(port_addr = 'COM20')
    #nj_port = test.devices[0]
